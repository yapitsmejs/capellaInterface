...
- Static catalog (no spatial query API) → bbox/point filtering is **client-side** (iterate items, shapely intersection).

Locked decisions: location = bbox/point only (no name menu); selection = contiguous index range; confirm = list-then-confirm default with `--yes`; non-interactive = prompts **and** flags share one code path; output = flat `<dest>/<item_id>/`; filters = bbox, datetime, instrument mode.
...
capella search --bbox MINLON,MINLAT,MAXLON,MAXLAT | --point LON,LAT
              ...
              [--limit N] [--json]
```
- Searches the `slc` collection, filters client-side by bbox/point intersection + datetime + mode, sorts by datetime ascending, prints a numbered table (`#  ID  datetime  mode  pol  size`). `--json` emits machine-readable rows for piping. Zero matches → clear message, exit non-zero. Used to learn item IDs / counts before downloading; does not write files.
...
2. **Search.** `catalog.find_slc_items(bbox|point=…, datetime=…, mode=…)` → datetime-ascending SLC item list.
...
- **`src/capella/catalog.py`** — add/extend:
  - `find_slc_items(*, bbox=None, point=None, datetime=None, instrument_mode=None, max_items=None)` — load the `capella-open-data-slc` collection via `pystac`, iterate `get_all_items()` (lazy), filter by shapely bbox/point intersection, `item.datetime`, `item.properties.get("capella:instrument_mode")`; return a datetime-ascending **list** (numbering needs materialization). `point` becomes a shapely point tested with `intersects`.
...
- **`src/capella/cli.py`** — implement `search` and `download` Click commands:
  ...
  - `search` prints the table / `--json`.
  - `--help` documents the guided flow + the SLC-only constraint (derived levels point at `capella l2`/`l2lcr`/`l2ml`).
...
- **`tests/test_catalog.py`** — add bbox/point/datetime/mode filtering + datetime-ascending sort on a fixture collection.
...
1. `uv run pytest -q` green, including new `test_cli.py` (CliRunner headless flow + `--yes`), `parse_range` cases, flat-layout download, and catalog bbox/point/mode filtering. Network tests auto-skipped offline.
...
## Files touched
```
src/capella/catalog.py     (find_slc_items w/ bbox+point+datetime+mode; item_download_size)
src/capella/download.py    (parse_range, format_item_table, flat download_item, total_size)
src/capella/cli.py         (search + download commands, guided interactive flow)
src/capella/levels.py      (unchanged — SLC-only)
tests/test_cli.py          (new — CliRunner flow, --yes, zero-match, --level hint)
tests/test_download.py     (parse_range + flat layout + idempotent skip)
tests/test_catalog.py      (bbox/point/datetime/mode filter + sort)
```